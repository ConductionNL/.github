...
> 💡 Skills live in two repos. Hydra's own `.claude/skills/` ships the pipeline-side workflow (opsx-_, hydra-gate-_, team-_, test-_, journeydoc-_, utilities). The `concurrentie-analyse` repo ships the upstream research and app-pipeline catalogue (specter-_, tender-_, ecosystem-_, app-_, swc-_).
...
`/opsx-new` → `/opsx-ff` _or_ `/opsx-continue` → `/opsx-plan-to-issues` → `/opsx-apply` → `/opsx-verify` → `/opsx-archive`
...
| `/opsx-plan-to-issues` | Convert tasks.md into tracking issues with proper labels (per-repo platform — GitHub primary, GitLab alternative, Forgejo only for non-Conduction repos) |
