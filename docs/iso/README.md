...
These may earn their own files in this folder later (`security.md` for 27001, `bio.md`, etc.). For now `code.md` is the only resident — the rest is tracked in [reference_conduction-roadmap.md](../ROADMAP.md) and in the memory index under _Tender Analysis & Compliance_.
...
5. **Gap & how to close it** — only present when coverage is not _Full_. A concrete next step, not a vague aspiration.
...
- **No aspirational claims.** If an ADR exists but is not enforced, mark the clause _Partial_ and describe the enforcement gap.
