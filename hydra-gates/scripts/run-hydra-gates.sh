...
# same symptom — an `occ upgrade` that reports complete success over an instance
# where nothing happened — and an app can pass either while failing the other.
#
# Nextcloud gates BOTH migrations and upgrade-time repair steps on one string:
# `<version>` in appinfo/info.xml, compared against the `installed_version` it
# recorded. Equal versions mean "already up to date". `occ upgrade` answers
# "No upgrade required.", exits 0, and never reads the files.
#
# MEASURED (openregister#3451, NC 34.0.3, from the release tarball): a migration
# added with `<version>` left alone created no table and wrote no row to
# oc_migrations; changing NOTHING but `<version>` ran it.
#
# 🔴 `occ migrations:status` CANNOT WARN YOU, so "check before you ship" is not
# a workable substitute. Its `Pending Migrations` field is built under
# `if ($migration->name())` and `SimpleMigrationStep::name()` returns '' — which
# every one of ours inherits — so it reads `None` unconditionally. Two of its
# other counters compare version strings against the integers 0..n and report
# the full count every time. Only Executed vs Available is honest.
#
# AND IT HAS BITTEN IN BOTH SHAPES. openregister found it with a migration.
# dossiq shipped `lib/Repair/RealignStatutoryVocabulary` — the step that puts
# corrupted confidentiality values back — with `<version>` untouched, so
# `occ upgrade` skipped dossiq entirely and 18 cases plus 7 caseTypes stayed
# corrupted on a live instance.
#
# THE SUBJECT IS THE RUNNABLE SET, NOT THE DIRECTORY. Only two kinds of added
# file run on upgrade: a `Version<n>Date<n>` class under lib/Migration/ (the one
# shape core discovers), and any class under lib/Migration/ or lib/Repair/ named
# in <pre-migration> or <post-migration>. decidiq ships sixteen
# `lib/Migration/Migrate*.php` classes core ignores — they run as registered
# repair steps — beside a trait and a helper. Demanding a version bump for a
# trait is a finding nobody can act on. <install> is excluded: a fresh install
# has no installed_version, so no bump would deliver it.
#
# DIFF-SCOPED, on `HAVE_DELTA_BASE` and NOT `SCOPE_TO_DIFF` — the mistake
# gate-98 shipped, where the `else` fired on every ordinary run because full
# scope is the default. A full-tree version would demand a version bump on every
# branch cut before this gate, for steps their authors never touched. The eight
# apps carrying stranded steps on 2026-09-05 are being fixed by name in their
# own pull requests; that debt is real and it is not an unrelated PR's to pay.
#
# NOT IN _ARM6_ALLOWED, deliberately: this gate READS a delta, so an empty scope
# must skip, never pass.
# ---------------------------------------------------------------------------
_mvb_log=${HYDRA_GATE_LOG_DIR}/hydra-gate-migration-version-bump.log
: > "${_mvb_log}"
...
# GATE 111 — flow-node-taxonomy
#
# A flow node that does not say what kind of step it is.
#
# WHY THIS BLOCK IS BEING ADDED LATE. #698 shipped gate-111 complete except for
# this: the checker, its acceptance fixtures, its expect.conf row and its own
# test suite all landed, and nothing ever called it. So it emitted no verdict
# in any repository, and `test_gate_acceptance_matrix.sh` failed with the only
# sentence that fits:
#
#   FAIL — [flow-node-taxonomy/planted] gate-111 emitted NO verdict line at
#          all — a gate that says nothing is not a pass
#
# That failure fails the `Package invariants` job, which is a REQUIRED check on
# main, so an unregistered gate has been blocking every pull request to this
# repository since #698 merged.
#
# SCOPED BY PATH, NEVER BY INTERFACE — #698's own design note, and the reason
# the prerequisite below tests for the directory rather than for IFlowNode. On
# a measured instance 38 of 65 step types are contributed by apps in OTHER
# repositories on their own release cycles. An interface-scoped gate would fire
# on all 38, inside pull requests that cannot fix them.
#
# See scripts/lib/check_flow_node_taxonomy.py.
# ---------------------------------------------------------------------------
if [ -d lib/Service/Flow/Nodes ]; then
    _fnt_log=${HYDRA_GATE_LOG_DIR}/hydra-gate-flow-node-taxonomy.log
    : > "${_fnt_log}"
    _fnt_helper="${SCRIPT_DIR}/lib/check_flow_node_taxonomy.py"
    if [ ! -f "${_fnt_helper}" ]; then
        _skip 111 "flow-node-taxonomy" wiring "check_flow_node_taxonomy.py not found at ${_fnt_helper} — lib/Service/Flow/Nodes exists here and NO node was inspected, so their declared kind and category are UNVERIFIED by this run."
    else
        set +e
        python3 "${_fnt_helper}" . >> "${_fnt_log}" 2>&1
        _fnt_rc=$?
        set +e
        # THE TERMINAL SUMMARY IS THE WIRING CHECK, and the checker says so in
        # its own source: without it the runner cannot tell a clean run from a
        # crashed one. Asserted before the exit code is believed.
        if ! grep -qE '^checked [0-9]+ flow node' "${_fnt_log}" 2>/dev/null; then
            _skip 111 "flow-node-taxonomy" wiring "check_flow_node_taxonomy.py exited ${_fnt_rc} without printing its terminal 'checked N flow node' summary, so NO node was judged and their taxonomy is UNVERIFIED by this run. See ${_fnt_log}."
        else
            case "${_fnt_rc}" in
                0)
                    _pass 111 "flow-node-taxonomy"
                ...
                1)
                    cat "${_fnt_log}"
                    _fnt_n=$(grep -cE '^FAIL ' "${_fnt_log}" 2>/dev/null || echo 0)
                    _fail 111 "flow-node-taxonomy" "${_fnt_n} flow node(s) that do not declare what kind of step they are — a BPMN export serves them as serviceTask/other, which looks answered and is not. See ${_fnt_log}"
                    ;;
                4)
                    _skip 111 "flow-node-taxonomy" na "no lib/Service/Flow/Nodes in this repository — it contributes no flow node of its own, and a node it contributes from elsewhere is not this gate's business (#698)."
                    ;;
                *)
                    _skip 111 "flow-node-taxonomy" wiring "check_flow_node_taxonomy.py exited ${_fnt_rc}, which is not one of its documented statuses (0 pass, 1 fail, 4 not applicable). NO verdict was produced. See ${_fnt_log}."
                    ;;
            esac
        fi
    fi
