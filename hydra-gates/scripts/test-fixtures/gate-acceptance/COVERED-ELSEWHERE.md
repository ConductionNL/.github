...
| gate-47 | security-change-has-tests | `test_gate_discarded_counts_and_empty_deltas.sh` | scaffolded in-suite — a real base ref plus two heads: a docs-only delta (must decline, `.github#401(b)`) and a delta dropping an auth attribute with no test co-change (must still FAIL) |
