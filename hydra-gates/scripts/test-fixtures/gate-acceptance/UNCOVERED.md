...
| gate-83 | contract-surface-shift | needs-base-ref | The gate's subject is a TRANSITION — a method moving between the magic (`@method`) and declared surfaces of a published contract — so it is meaningless without two commits to compare. A planted/clean pair is two directory states with no history between them, and this driver hands the runner no base, so both arms would report `EMPTY SCOPE` and the planted assertion would grade a scope fault as a gate defect. Covered instead by `scripts/lib/test_check_contract_surface_shift.sh` (9 assertions, both shift directions, the annotation grammar, unresolvable-base-fails-closed), which builds real git fixtures with `git init` + two commits. Additionally verified end to end against the commit that caused the outage it was written for: replayed against `openregister@83e8798a4`, the gate FAILs and names all five shifted methods, and reports NOT APPLICABLE on a diff touching no contract file. Bundling it here needs the driver to grow a per-bundle git history, which is the same gap gate-52's count-ratchet half records. |
