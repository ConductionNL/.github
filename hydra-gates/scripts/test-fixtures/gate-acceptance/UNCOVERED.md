...
|---|---|---|---|
| gate-4 | composer-audit | needs-external | Requires the `composer` binary on PATH **and** a network round-trip to the Packagist security-advisories API, since `composer audit --locked` resolves advisories remotely. A planted arm would additionally need a lock pinning a package with a live published CVE, which rots as advisories are superseded. The gate's verdict logic — a named advisory FAILs on the first attempt, a transport failure (exit 100) is retried three times and is still FAIL if the feed never answers — is covered without the network by `scripts/lib/test_gate_4_composer_audit_transport_retry.sh`, which stubs `composer` on PATH. |
...
| gate-83 | contract-surface-shift | needs-diff | The gate's subject is a TRANSITION — a method moving between the magic (`@method`) and declared surfaces of a published contract — so it is meaningless without two commits to compare. A planted/clean pair is two directory states with no history between them, and this driver hands the runner no base, so both arms would report `EMPTY SCOPE` and the planted assertion would grade a scope fault as a gate defect. Covered instead by `scripts/lib/test_check_contract_surface_shift.sh`, which builds real git fixtures with `git init` + two commits: 9 assertions covering both shift directions, the `@contract-shift` annotation grammar (including a bare tag with no reason, which caught a live regex bug where `\s*` crossed the newline and captured the docblock's `*/` as the reason), empty scope, not-applicable, and unresolvable-base-fails-closed. Additionally replayed end to end against `openregister@83e8798a4` — the commit that broke opencatalogi and decidesk — where the gate FAILs and names all five shifted methods, and reports NOT APPLICABLE on a diff touching no contract file. Bundling it here needs the driver to grow a per-bundle git history, the same gap gate-52's count-ratchet half records. |
...
| gate-68 | duplicate-index-pages | needs-diff | The gate's ENTIRE subject is a base-vs-head ratchet (design.md Decision 3, modeled on gate-52) — the count of `type:"index"` pages per `(register, schema)` pair at BASE_REF vs HEAD — so a planted/clean directory PAIR with no history between them cannot exercise the FAIL/WARN split at all: this driver's `_run` never passes `--scope-to-diff` or `--base` to the wrapper (verified 2026-08-19: `register-dialect`'s own expect.conf notes the same constraint — "the acceptance driver runs unscoped"), and gate-68's shell block only passes `--base-ref` to the checker when `SCOPE_TO_DIFF=1` (mirrors gate-52's own `if [ "${SCOPE_TO_DIFF}" = "1" ]` gate exactly). Without a base, EVERY `>1` pair reports WARN by design (never FAIL), so a planted/clean pair here could show the STATE half (a manifest with a duplicate is silent vs one without) but never the ratchet's FAIL half — the same gap gate-52's own count-ratchet row above records, one line up. Covered instead by `scripts/lib/test_check_duplicate_index_pages.js`, which drives the checker directly (not through the shell wrapper) against `scripts/test-fixtures/duplicate-index-pages/{good,broken,case-varied}/` for the grouping/state half (broken/ spreads one 3-page group across a base + two `manifest.d/` fragments — the fragment-merge path; case-varied/ pins that `config.register`/`config.schema` is case-normalized before grouping, per this gate's binding orchestrator ruling) AND builds five real two-commit git fixtures (via `assembleAtRef`, `git init` + two commits, same shape as gate-83's suite) covering every row of design.md Decision 3's table: `1→2` and `6→7` FAIL and NAME the pair; `6→6` (unchanged) and `4→3` (shrank, still >1) WARN and do NOT fail; `2→1` (resolved) reports nothing at all. Additionally replayed end to end through the REAL shell wrapper (`run-hydra-gates.sh --scope-to-diff --base <ref>`) against a synthetic two-commit fixture (FAIL, exit non-zero, naming the pair) and against the live `apps-extra/shillinq` checkout (27 pairs / 64 pages, exit 0, WARN-only with no base; `--base-ref HEAD` against the same unmodified checkout — all 27 WARN, exit 0; a planted 28th duplicate on a previously-clean pair — FAIL, exit 1, naming exactly that pair, the other 27 unaffected). Bundling the ratchet half here needs the driver to grow a per-bundle git history, same as gate-83 and gate-52. |
