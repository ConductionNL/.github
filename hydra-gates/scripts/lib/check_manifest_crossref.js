...
function main() {
	...
	// (g) and (h) are ADVISORY. Every finding they produce is a WARN, which
	// never sets the exit code, and a crash inside either one is reported as a
	// WARN too. Both checks read PHP with regexes across all fleet repos at once,
	// so they ship warn-only until their findings have been measured on every
	// repo. Promote a check to fail() only after that measurement.
	try {
			...
			if (!resolver) {
				...
				for (const { ptr, id, literal } of roleLiterals) {
					if (!resolver.roles.has(literal)) {
						warn('role-resolvable', ptr, `menu item '${id || '(no id)'}' names role '${literal}' in visibleIf.user.primaryRole, which ${path.relative(APP_DIR, resolver.file)}'s resolver can never emit`)
					}
				}
			}
		}
	} catch (e) {
		warn('role-resolvable', '/', `check could not run, so nothing was judged: ${e && e.message ? e.message : e}`)
	}
		...
		if (groupCallSites.length > 0) {
			...
			for (const site of groupCallSites) {
				...
				if (!declaredGroups.groups.has(site.literal)) {
					warn('group-declared', `${rel}:${site.line}`, `isInGroup($uid, '${site.literal}') names a group id declared nowhere in this app's RBAC configuration (register/schema authorization blocks or the OAS scope map)`)
				}
			}
		}
	} catch (e) {
		warn('group-declared', '/', `check could not run, so nothing was judged: ${e && e.message ? e.message : e}`)
	}
