...
"""Tests for gate-114's checker (check_stale_fleet_app_id.py).
...
class StaleFleetAppIdTest(unittest.TestCase):
    ...
    def test_a_register_slug_is_reported_separately_not_silently_dropped(self):
        """Exclusion 4 was wrong once and this arm is what stops it recurring.

        It began as a plain exclusion citing integriq's own docblock saying the
        `openconnector` slug is "deliberately NOT renamed". That docblock is
        stale: integriq ships `MigrateRegisterSlug` mapping openconnector to
        integriq, and its register json declares slug "integriq". So slugs move,
        and a consumer pinned to the old one reads zero rows on a migrated
        instance. They stay out of the main COUNT, because the fix is a
        migration probe rather than a literal swap and fleet policy still
        records slugs as frozen, but they must be PRINTED.
        """
        rc, out = self.run_check({
        ...
        self.assertEqual(rc, 0, f"slugs are not counted as stale names\n{out}")
        self.assertIn("register slug reference(s)", out, out)
        self.assertIn("MigrateRegisterSlug", out,
                      f"the advisory must state why the old rationale is stale\n{out}")
        self.assertEqual(
