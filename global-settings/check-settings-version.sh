...
if $online_fetch_ok && semver_gt "$online_version" "$installed_version"; then
    ...
    echo "  3. Then say: \"update my global settings to ${online_version}\""
    echo "  4. After Claude finishes, re-apply the immutable lock in your own terminal:"
    echo "       sudo chattr +i \$HOME/.claude/settings.json \$HOME/.claude/hooks/*.sh \$HOME/.claude/settings-version"
    echo "     Leave \$HOME/.claude/settings.json out of that list if you switch models with the VSCode"
    echo "     model picker — the extension rewrites it on every switch and a locked one fails with"
    echo "     EPERM. The trade-off, and the alternative that keeps all four layers (pin the model in"
    echo "     project-local settings), are in docs/claude/global-claude-settings.md#troubleshooting."
    ...
    if [ "$online_source" = "github-raw" ]; then
        ...
        echo ""
        echo "    # sound-notify.sh (added in v2.2.0)"
        echo "    content=\$(curl -fsSL --max-time 10 'https://raw.githubusercontent.com/${online_repo_slug}/${tracking_ref}/global-settings/sound-notify.sh')"
        printf '%s\n' "    printf '%s\\n' \"\$content\" > \"\$HOME/.claude/hooks/sound-notify.sh\""
        echo "    chmod 555 \"\$HOME/.claude/hooks/sound-notify.sh\""
        echo ""
        echo "    # user-hooks-dispatch.sh (added in v2.4.0)"
        echo "    content=\$(curl -fsSL --max-time 10 'https://raw.githubusercontent.com/${online_repo_slug}/${tracking_ref}/global-settings/user-hooks-dispatch.sh')"
        printf '%s\n' "    printf '%s\\n' \"\$content\" > \"\$HOME/.claude/hooks/user-hooks-dispatch.sh\""
        echo "    chmod 555 \"\$HOME/.claude/hooks/user-hooks-dispatch.sh\""
        echo ""
        ...
        echo ""
        echo "    # sound-notify.sh (added in v2.2.0)"
        echo "    git -C '${REPO_DIR}' show 'origin/main:${_rel_base}/sound-notify.sh' > \"\$HOME/.claude/hooks/sound-notify.sh\""
        echo "    chmod 555 \"\$HOME/.claude/hooks/sound-notify.sh\""
        echo ""
        echo "    # user-hooks-dispatch.sh (added in v2.4.0)"
        echo "    git -C '${REPO_DIR}' show 'origin/main:${_rel_base}/user-hooks-dispatch.sh' > \"\$HOME/.claude/hooks/user-hooks-dispatch.sh\""
        echo "    chmod 555 \"\$HOME/.claude/hooks/user-hooks-dispatch.sh\""
        echo ""
    ...
    echo "immutable protection is gone until next session. Claude can run neither 'sudo chattr' nor a"
